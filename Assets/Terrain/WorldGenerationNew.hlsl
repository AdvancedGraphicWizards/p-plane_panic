...
float CanyonCarve(float2 p, float canyonWidth, float canyonBaseWidth, float canyonDepth, float axisOffset, float period, float amplitude, float periodOffset) {
    ...
    sinSDF = (pow(Smin(canyonWidth, max(0, sinSDF -canyonBaseWidth), 1), 2) / pow(canyonWidth, 2));
    sinSDF = 1 - sinSDF;
...
float SampleHeight(float x, float z) {
	...
	// Get canyon depth
	float sinSDF = CanyonCarve(float2(x, z), 20, 5, 36, 0, 500, 100, 0);
	sinSDF += CanyonCarve(float2(x, z), 30, 50, 30, 0, 500, 100, 0);
	sinSDF += CanyonCarve(float2(x, z), 70, 80, 20, 0, 500, 100, 0);

    // Decrease the height of the terrain in the canyon
    height *= min(1, 40 / sinSDF);
