...
void SampleTerrainHeight_float(float x, float z, float2 offset, out float3 tHeight) {
    x += offset.x;
    z += offset.y;
    ...
    tHeight.y = FBMErosion(float2(x / 300, z / 300), 10).x * 150;

    tHeight.y += abs(FBMErosion(float2(x / 2000, z /2000), 10).x * 2000) * min(1, abs(x) / 4000);
